...
const IconData = GObject.registerClass({
    ...
    get at_name() {
        if (this._at_name === 'undefined') this._at_name = null;
        return this._at_name
    }

    set at_name(value) {
        if (this.at_name == value)
            return;
...
class AppIndicatorCustomIconPage extends Adw.PreferencesPage {
    _init(settings, settingsKey) {
        ...
        this.listStore.append(new IconData({id: "", name: "", at_name: ""}));
        ...
        factoryId.connect('setup', (widget, item) => {
            const label = new Gtk.EditableLabel({text: "",});
            item.set_child(label);
        });
        factoryId.connect('bind', (widget, item) => {
            ...
            label.connect('changed', (w, ud) => {
                this._autoSave(item);
            });
        });

        const factoryName = new Gtk.SignalListItemFactory();
        factoryName.connect('setup', (widget, item) => {
            const label = new Gtk.EditableLabel({text: ""});
            item.set_child(label);
        });
        factoryName.connect('bind', (widget, item) => {
            const label = item.get_child();
            const data = item.get_item();
        ...
        factoryItName.connect('setup', (widget, item) => {
            const label = new Gtk.EditableLabel({text: ""});
            item.set_child(label);
        });
        factoryItName.connect('bind', (widget, item) => {
            const label = item.get_child();
            const data = item.get_item();
            label.set_text(data.at_name);
    ...
    _autoSave(item) {
        ...
        for (let i=0; i<storeLength; i++) {
            ...
            const atName = currentItem.at_name;
            if (id && name ) {
                newStore.push([id, name, atName || ""]);
            } 
        }
  
        this._settings.set_value(this._settingsKey.CUSTOM_ICONS, new GLib.Variant(
                        'a(sss)', newStore));
